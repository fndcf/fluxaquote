...
describe("Theme", () => {
  describe("colors", () => {
    it("deve ter cores primárias definidas", () => {
      ...
      expect(theme.colors.primaryDark).toBe("#047857");
      expect(theme.colors.primaryLight).toBe("#10B981");
    });

    it("deve ter cores secundárias definidas", () => {
      expect(theme.colors.secondary).toBe("#0F172A");
      expect(theme.colors.secondaryLight).toBe("#1E293B");
    });
